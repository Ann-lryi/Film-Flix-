        ...
        try {
            ...
            val token = extractToken(html)
                ?: return@withContext fail(
                    "extract_token",
                    "Không tìm thấy data-obf/sUb hay pattern token nào trong HTML - site có thể đã đổi cấu trúc trang embed",
                )
            ...
            Log.d(TAG, "B4 proxy cục bộ sẵn sàng: ${proxy.base}")
            Result.Success("${proxy.base}/stream.m3u8")
        } catch (e: Exception) {
